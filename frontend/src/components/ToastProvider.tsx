...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      {children}
      {/* aria-live so "added to cart" and error messages are announced to
          screen-reader users, who never see the visual toast. */}
      <div
        role="status"
        aria-live="polite"
          ...
          {toasts.map((toast) => (
            <motion.div
              ...
              className={`pointer-events-auto flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium text-white shadow-lg ${
                toast.variant === "success" ? "bg-maroon-800" : "bg-red-600"
              }`}
            >
